...
* **In flight:** the money model is being replaced — per-chore allowance out, one household-wide weekly pot apportioned by days present and split within each day by points. Spec with every decision settled: `docs/Plans/weekly-allowance-and-goals.md`. **Hard privacy rule that comes with it:** no kid may see another kid's earnings, ceiling, or who set the week's divisor. Rotation groundwork (least-recently-served) has already landed.
...
* **Rooms & chore library:** `rooms` table + nullable `chores.room_id` (null = "General") + `chores.icon`. Rooms are purely organizational — generation/rotation/payout never read them; `deleteRoom` releases its chores to General. The built-in catalog ships as CODE (`src/lib/choreLibrary.ts`), never as DB rows — a fresh household starts empty. `library.ts` multi-adds templates UNASSIGNED with allowance 0 (money is family policy; templates carry points only) and skips titles already in the room. Unassigned chores are legal everywhere (generation skips them — that's the "stock the house first, assign later" flow). Person-centric assignment lives in `assignments.ts` (fixed = replace assignee, rotating = append to pool; both drop future pending + regenerate). `/admin/chores/new` uses NAMED actions (`?/custom`, `?/library`) — SvelteKit forbids mixing default + named.
